...
public class GenericUserDAO extends AutoKeys implements UserDAO {
	...
	private static LoginAuthenticator loginAuthenticator = null;
	...
	public User selectById(int userId) {
		System.out.println("--> [GenericUserDAO.selectById] ......");
		String q = SystemGlobals.getSql("UserModel.selectById");
		System.out.println("DEBUG: Calling the SQL Query for 'UserModel.selectById' ... userId = " + userId);
		PreparedStatement p = null;
		ResultSet rs = null;
		try {
			...
			if (rs.next()) {
				...
				System.out.println("DEBUG: u.privateMessagesCount = " + u.getPrivateMessagesCount());
				rs.close();
				p.close();
				// User groups
				p = JForumExecutionContext.getConnection().prepareStatement(SystemGlobals.getSql("UserModel.selectGroups"));
				System.out.println("DEBUG: Calling the SQL Query for 'UserModel.selectGroups' ... userId = " + userId);
				p.setInt(1, userId);
				rs = p.executeQuery();
				while (rs.next()) {
					Group g = new Group();
					...
					System.out.println("DEBUG: g.id = " + g.getId() + ", g.name = " + g.getName());
					u.getGroupsList().add(g);
				}
			}
			System.out.println("DEBUG: the object of 'u' ==> [" + u + "]");
			return u;
		} catch (SQLException e) {
			throw new DatabaseException(e);
		} finally {
	...
	protected void fillUserFromResultSet(User u, ResultSet rs) throws SQLException {
		System.out.println("--> [GenericUserDAO.fillUserFromResultSet] ......");
	...
	public int addNew(User user) {
		...
		try {
			...
			addToGroup(id, new int[] { SystemGlobals.getIntValue(ConfigKeys.DEFAULT_USER_GROUP) });
			user.setId(id);
			return id;
		} catch (SQLException e) {
			throw new DatabaseException(e);
	...
	public void addNewWithId(User user) {
		PreparedStatement p = null;
		try {
			p = getStatementForAutoKeys("UserModel.addNewWithId");
			initNewUser(user, p);
	...
	public List selectAllWithKarma(int startFrom, int count) {
		return loadKarma(this.selectAll(startFrom, count));
	}
	...
	public User getLastUserInfo() {
		System.out.println("--> [GenericUserDAO.getLastUserInfo] ......");
		PreparedStatement p = null;
		ResultSet rs = null;
		try {
			User u = new User();
			System.out.println("INFOR: Calling SQL Query for 'UserModel.lastUserRegistered' ...");
			p = JForumExecutionContext.getConnection().prepareStatement(SystemGlobals.getSql("UserModel.lastUserRegistered"));
			rs = p.executeQuery();
			rs.next();
			u.setUsername(rs.getString("username"));
			u.setId(rs.getInt("user_id"));
			System.out.println("DEBUG: UserModel.lastUserRegistered = " + u.getUsername() + ", userId = " + u.getId());
			return u;
		} catch (SQLException e) {
			throw new DatabaseException(e);
		} finally {
	...
	public int getTotalUsers() {
		System.out.println("--> [GenericUserDAO.getTotalUsers] ......");
		...
		try {
			...
			int totalusers = getTotalUsersCommon(preparedStatement);
			System.out.println("DEBUG: UserModel.totalUsers = " + totalusers);
			return totalusers;
		} catch (SQLException e) {
			throw new DatabaseException(e);
		} finally {
	...
	public String getUserAuthHash(int userId) {
		System.out.println("--> [GenericUserDAO.getUserAuthHash] ......");
		PreparedStatement p = null;
		ResultSet rs = null;
		try {
			System.out.println("DEBUG: Calling the SQL query for 'UserModel.getUserAuthHash' ... , userId = " + userId);
			...
			System.out.println("DEBUG: the hashValue of userId (" + userId + ") = " + hash);
			return hash;
		} catch (SQLException e) {
			throw new DatabaseException(e);
		} finally {
